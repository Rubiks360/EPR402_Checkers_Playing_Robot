...
class DPiece(object):
    ...
    def _check_promote(self, drow):
        ...
        '''
        if not self.is_king:
            if (self.color == 'LIGHT' and drow == 0) or (self.color == 'DARK' and drow == 7):
                return True
        return False
    ...
    def _possible_action_king(self):
        ...
        for dir in direction:
            ...
            # test the possible jump position
            if (trow_2 >= 0) & (trow_2 <= 7) & (tcol_2 >= 0) & (tcol_2 <= 7):
                # get piece to jump over
                piece_jump = board.get_piece(trow_1, tcol_1)
                valid_jump = False
                # check for valid jump
                if piece_jump is not None:
                    if piece_jump.color != self.color:
                        valid_jump = True

                # test if open position after jump and valid jump
                if board.is_free(trow_2, tcol_2) & (valid_jump is True):
                    move.append(DAction('CAPTURE', (row, col), (trow_2, tcol_2), board.get_piece(trow_1, tcol_1)))
                    capture = True

            # test for valid position and open position and not yet captured action made
            if (trow_1 >= 0) & (trow_1 <= 7) & (tcol_1 >= 0) & (tcol_1 <= 7) & (board.is_free(trow_1, tcol_1)) & (capture is False):
                move.append(DAction('MOVE', (row, col), (trow_1, tcol_1)))

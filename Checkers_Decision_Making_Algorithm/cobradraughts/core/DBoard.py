...
class DBoard(object):
    ...
    def __init__(self):
        ...
        self.light_pieces = []
        self.dark_pieces = []
        self.bitmap = [None] * 32      # None = empty
        self.movelist = []
    ...
    def is_free(self, row, column):
        ...
        '''
        if row < 0 or row >= 8:  #10
            return False
        if column < 0 or column >= 8:  #10
            return False
    ...
    def apply_action(self, action, chain=False):
        ...
        if piece is None:
            raise Exception("NO PIECE IN SOURCE! ", srow, scol, "with action", action)
